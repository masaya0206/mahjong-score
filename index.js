...
$("#createRoomBtn").addEventListener("click", async ()=>{
  ...
  try{
    ...
    rememberRoom(row.room_id,row.room_code,row.host_key,row.member_id);
...
$("#joinRoomBtn").addEventListener("click", async ()=>{
  ...
  try{
    ...
    if(error) throw error;
    const row=Array.isArray(data)?data[0]:data;
    rememberRoom(row.room_id,code,null,row.member_id);
    location.href=`./room.html?id=${encodeURIComponent(row.room_id)}`;
  }catch(e){ msg.textContent=e.message; }
});
...
function renderRecent(){
  ...
  box.innerHTML="";
  if(!items.length){
    box.innerHTML=`<p class="muted">まだありません。</p>`;
    return;
  }
  items.forEach(x=>{
...
if(!isConfigured){
  msg.textContent="Supabase接続前です。config.js を設定してください。";
}
renderRecent();
